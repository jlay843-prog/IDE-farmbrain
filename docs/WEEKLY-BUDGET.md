...
Catch-up (2026-09-17, this tree): W14 terminal pane, W15 Legion `/forge` Telegram alias, W16 docs, Aether reads `forge-handoff.json`, PATH-less Python launch, leftover v1 project-row/smoke polish. Post-W16 next four (2026-09-18): Monaco vendored offline (`vendor:monaco`), health/smoke checks; Telegram live poll and code signing still blocked. Post-W16 batch two (2026-09-18): Phase I CLI polish (`forge health`, `forge log`), packaged smoke parity (`npm run smoke:packaged`); Telegram live poll and code signing still blocked. Post-W16 batch three (2026-09-18): shortcut `forge.ico`, `launch-forge.ps1` via `forge.cmd`, `npm run smoke:all`, smoke `/api/log` + `forge telegram --probe`; Telegram live poll and code signing still blocked. Post-W16 batch four (2026-09-18): launch `forge health` preflight, smoke `forge log --json` + `forge git --json`; Telegram live poll and code signing still blocked. Post-W16 batch five (2026-09-18): launch/smoke-all `forge health --json` preflight, smoke `forge health --json` parity vs `/api/health`, smoke `forge which --json`; Telegram live poll and code signing still blocked. Post-W16 batch six (2026-09-18): signing/remote prep docs, smoke `forge status --json` parity vs `/api/status`, GIT-REMOTE origin note, Git Show diff review-only title; Telegram live poll and code signing still blocked. Token schedule below is unchanged.
