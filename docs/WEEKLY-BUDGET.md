...
Catch-up (2026-09-17, this tree): W14 terminal pane, W15 Legion `/forge` Telegram alias, W16 docs, Aether reads `forge-handoff.json`, PATH-less Python launch, leftover v1 project-row/smoke polish. Post-W16 next four (2026-09-18): Monaco vendored offline (`vendor:monaco`), health/smoke checks; Telegram live poll and code signing still blocked. Post-W16 batch two (2026-09-18): Phase I CLI polish (`forge health`, `forge log`), packaged smoke parity (`npm run smoke:packaged`); Telegram live poll and code signing still blocked. Token schedule below is unchanged.
